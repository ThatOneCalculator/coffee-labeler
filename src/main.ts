...
jetstream.on("open", () => {
	logger.info(
		`Connected to Jetstream at ${FIREHOSE_URL} with cursor ${jetstream.cursor} (${epochUsToDateTime(jetstream.cursor as number)})`,
	);
...
jetstream.onCreate(
	...
	(event: CommitCreateEvent<typeof WANTED_COLLECTION>) => {
		...
		if (event.commit?.record?.subject?.uri?.includes(DID)) {
			label(
				event.did,
				event.commit.record.subject.uri.split("/").pop() as string,
			);
		}
	},
);
